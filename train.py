...
def train(model, dataloader, num_epochs, device, logger, filename):
    ...
    for epoch in range(num_epochs):
        ...
        logger.info(f"Epoch [{epoch+1}/{num_epochs}], Loss: {running_loss/len(train_loader):.4f}, Training Accuracy : {train_accuracy}")
...
if __name__ == "__main__":
    ...
    parser.add_argument('--seed', default=69, type=int, help='Random ssed.')
    ...
    with torch.cuda.device(args.gpu_index):
        ...
        model = train(model, dataloader, args.epochs, device, logger, filename)